  ...
  (
    ...
    useEffect(() => {
      if (open || peek === "none") {
        ...
        (peekTimerRef as any).current = null;
        setIsPeeking(false);
      }
    }, [open, peek]);
    ...
    const schedulePeek = useCallback(() => {
      cancelPeekTimer();
      (peekTimerRef as any).current = setTimeout(() => setIsPeeking(true), 150);
    }, [cancelPeekTimer]);
    const scheduleDismissPeek = useCallback(() => {
      cancelPeekTimer();
      (peekTimerRef as any).current = setTimeout(() => setIsPeeking(false), 250);
    }, [cancelPeekTimer]);
...
const SidebarShell = forwardRef<HTMLDivElement, SidebarShellProps>(
  ({ side, variant, bordered = true, rail = true, railTooltipOpen, className, children, ...props }, ref) => {
    ...
    useEffect(() => {
      ...
      (prevOpenRef as any).current = open;
      if (!isResizing) {
        setDragFlip(false);
        return;
      }
...
const SidebarRail = forwardRef<HTMLButtonElement, SidebarRailProps>(
  ({ className, tooltipOpen, ...props }, ref) => {
    ...
    const onPointerDown = (event: React.PointerEvent<HTMLButtonElement>) => {
      ...
      if (!panel) return;
      (dragRef as any).current = { startX: event.clientX, startWidth: panel.offsetWidth, moved: false, collapsed: false };
...
const SidebarGroup = forwardRef<HTMLDivElement, SidebarGroupProps>(
  (
    ...
    if (prevOpenRef.current !== open) {
      (prevOpenRef as any).current = open;
      (togglingRef as any).current = true;
    }
        ...
        inner = (
          <>
            ...
            <motion.div
              ...
              onAnimationComplete={() => {
                (togglingRef as any).current = false;
