...
function Select({
  ...
  const cancelAckClose = useCallback(() => {
    if (ackTimeoutRef.current !== null) {
      ...
      (ackTimeoutRef as any).current = null;
    }
  }, []);
  ...
  const handleValueChange = useCallback(
    (next: string) => {
      (lastPickRef as any).current = performance.now();
  ...
  const handleOpenChange = useCallback(
    (nextOpen: boolean) => {
      if (!nextOpen && performance.now() - lastPickRef.current < 100) {
        cancelAckClose();
        (ackTimeoutRef as any).current = window.setTimeout(() => {
          (ackTimeoutRef as any).current = null;
          setOpen(false);
        }, selectionAckMs);
        return;
                ...
                >
                  ...
                  {open && (
                    <AnimatePresence>
                      {checkedRect && (
                        <motion.div
                          className={`absolute ${shape.bg} bg-selected pointer-events-none`}
    ...
    return (
      <SelectPrimitive.Item
        ...
        className={cn(
          ...
          "transition-[color] duration-80 text-foreground",
