...
export function useDropdownSearchHost(
  ...
  const register = useCallback((handle: SearchHandle) => {
    (handleRef as any).current = handle;
    setSearchMounted(true);
    return () => {
      if ((handleRef as any).current === handle) {
        (handleRef as any).current = null;
...
const DropdownSearch = forwardRef<HTMLInputElement, DropdownSearchProps>(
  (
    ...
    (valueRef as any).current = value;
    const onValueChangeRef = useRef(onValueChange);
    (onValueChangeRef as any).current = onValueChange;
    const clearOnCloseRef = useRef(clearOnClose);
    (clearOnCloseRef as any).current = clearOnClose;
