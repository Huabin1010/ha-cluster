...
function useMenuScope(
  ...
  (orderedRowsRef as any).current = orderedRows;
  ...
  const syncRows = useCallback(() => {
    ...
    (orderedRowsRef as any).current = sorted;
    ...
    (registeredCountRef as any).current = sorted.length;
    recomputeActive();
...
function useMenuRow(rowRef: RefObject<HTMLLIElement | null>, isSubRow = false) {
  ...
  const setActive = useCallback(
    (active: boolean) => {
      (activeFlagRef as any).current = active;
