    ...
    return (
      ...
      >
        ...
        <input
          ...
          onFocus={(e) => {
            (interactingRef as any).current = true;
            e.currentTarget.select();
          }}
          onBlur={() => {
            (interactingRef as any).current = false;
            if (draft !== value) {
      ...
      <NumberField.Root
        ...
        onValueCommitted={(_, eventDetails) => {
          ...
          if (eventDetails.reason === "scrub") {
            (pointerDownRef as any).current = false;
      ...
      >
        <NumberField.ScrubArea
          ...
          onPointerDownCapture={() => {
            (pointerDownRef as any).current = true;
          }}
          onClick={() => {
...
const ColorPicker = forwardRef<HTMLDivElement, ColorPickerProps>(
  (
    ...
    useEffect(() => {
      ...
      if (!p) return;
      (oklchHueRef as any).current = null;
    ...
    const updateHsv = useCallback(
      (next: { h?: number; s?: number; v?: number; a?: number }) => {
        ...
        (lastEmittedRef as any).current = formatted;
    ...
    const root = (
      ...
      >
        ...
        <div className="flex flex-col [&>*]:mb-0 [&>*+*]:-mt-px">
          <HueSlider h={hsv.h} onChange={(h) => { (oklchHueRef as any).current = null; updateHsv({ h }); }} />
          <AlphaSlider
            a={hsv.a}
          ...
          onChannelChange={(channel, value) => {
            ...
            switch (channel) {
              ...
              case "hSL": case "sSL": case "lSL": {
                if (channel === "hSL") (oklchHueRef as any).current = null;
