...
const AskUserQuestions = forwardRef<HTMLDivElement, AskUserQuestionsProps>(
  function AskUserQuestions(
    ...
    useEffect(() => {
      (answersRef as any).current = answers;
    }, [answers]);
    ...
    const writeAnswers = useCallback(
      (
        ...
        (answersRef as any).current = next;
    ...
    useEffect(() => {
      (focusedIndexRef as any).current = focusedIndex;
    }, [focusedIndex]);
    ...
    const markFocusRestore = useCallback(() => {
      ...
      if (
        ...
        (restoreFocusRef as any).current = true;
      }
    }, []);
    useEffect(() => {
      if (!restoreFocusRef.current) return;
      (restoreFocusRef as any).current = false;
    ...
    const selectedGroups = useMemo(() => {
      ...
      (prevGroupMapRef as any).current = nextGroupMap;
      return groups;
    }, [selectedIndices]);
