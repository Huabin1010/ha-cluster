...
const CommandMenu = forwardRef<HTMLDivElement, CommandMenuProps>(
  (
    ...
    useIsoLayoutEffect(() => {
      (highlightRef as any).current = activeIndex;
    ...
    const scrollToRow = useCallback(
      (index: number, mode: "center" | "top") => {
        ...
        (scrollAnimationRef as any).current = null;
        if (mode === "top" || index === 0) {
        ...
        (scrollAnimationRef as any).current = animate(viewport.scrollTop, target, {
          ...spring.fast,
          onUpdate: (value) => {
    ...
    (onSelectRef as any).current = onSelect;
...
const CommandMenuTabs = forwardRef<HTMLDivElement, CommandMenuTabsProps>(
  ({ tabs, value, onValueChange, className, children, ...props }, ref) => {
    ...
    useIsoLayoutEffect(() => {
      (tabsRef as any).current = { tabs, value, onValueChange };
      return () => {
        (tabsRef as any).current = null;
      };
...
const CommandMenuList = forwardRef<HTMLDivElement, CommandMenuListProps>(
  ({ className, children, renderItem, ...props }, ref) => {
    ...
    if (fill && fill.activeIndex !== null) (lastActiveRef as any).current = fill.activeIndex;
...
function CommandMenuDialog({
  ...
  (openRef as any).current = open;
  const onOpenChangeRef = useRef(onOpenChange);
  (onOpenChangeRef as any).current = onOpenChange;
  ...
  (controlledRef as any).current = openProp !== undefined;
