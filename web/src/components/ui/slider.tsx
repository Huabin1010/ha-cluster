...
const CompactSlider = forwardRef<HTMLDivElement, SliderEngineProps>(
  (
    ...
    (valuesRef as any).current = values;
    (minRef as any).current = min;
    (maxRef as any).current = max;
    ...
    useEffect(() => {
      if (isHovered) {
        (hoverDelayRef as any).current = setTimeout(() => setShowHoverTooltip(true), 100);
      } else {
    ...
    useLayoutEffect(() => {
      ...
      (trackWidthRef as any).current = w;
