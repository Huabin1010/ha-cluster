...
export function useKeyboardNavGate(open: boolean) {
  ...
  useEffect(() => {
    ...
    (keyboardNavRef as any).current = active instanceof HTMLElement && active.matches(":focus-visible");
  }, [open]);

  const trackKeyboardNav = useCallback((e: KeyboardEvent) => {
    if (POPUP_NAV_KEYS.includes(e.key)) (keyboardNavRef as any).current = true;
  }, []);
