...
export function useFluidHover<T extends HTMLElement>(
  ...
  (activeIndexRef as any).current = activeIndex;
  ...
  const runMeasurement = useCallback(() => {
    ...
    if (changed) {
      (itemRectsRef as any).current = rects;
      setItemRects(rects);
    }
    return true;
  }, [containerRef]);
  ...
  const scheduleMeasurement = useCallback(
    (attemptsLeft: number) => {
      ...
      (remeasureRafIdRef as any).current = requestAnimationFrame(() => {
        (remeasureRafIdRef as any).current = null;
        if (runMeasurement()) {
          setIsMeasured(true);
  ...
  const getItemRo = useCallback(() => {
    if ((itemRoRef as any).current === null && typeof ResizeObserver !== "undefined") {
      (itemRoRef as any).current = new ResizeObserver(() =>
        scheduleMeasurement(measurementAttempts)
      );
    }
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent) => {
      ...
      (rafIdRef as any).current = requestAnimationFrame(() => {
        (rafIdRef as any).current = null;
        const container = containerRef.current;
        if (!container) return;
        setActiveIndex(
          pickNearest({
