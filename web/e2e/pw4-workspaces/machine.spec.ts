...
test.describe("PW-4 机器连接与域名", () => {
  test("PW4-23 @pw4 @smoke 详情页复制 HTTP 执行并接入域名", async ({ pageAs }) => {
    ...
    const cmd = page.getByTestId("ws-http-exec");
    await expect(cmd).toBeVisible();
    await expect(cmd).toContainText("/exec");
    await expect(page.getByTestId("ws-web-terminal")).toBeVisible();
    await page.getByTestId("ws-copy-http-exec").click();
