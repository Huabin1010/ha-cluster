...
func TestResolvePrefersFabric(t *testing.T) {
	w := models.Workspace{Status: models.WSRunning, SSHPort: 22001, Visibility: models.VisShared}
	n := models.Node{FabricIP: "10.88.0.10", LanIP: "192.168.1.10", BreakglassSSH: "vps:8092"}
	tg, err := Resolve(w, n, models.RoleDeveloper, true, false)
...
func TestResolveViewerDenied(t *testing.T) {
	...
	if _, err := Resolve(w, n, models.RoleViewer, false, false); err != ErrDenied {
		t.Fatalf("%v", err)
	}
}
...
func TestResolveStopped(t *testing.T) {
	...
	n := models.Node{FabricIP: "10.88.0.10"}
	if _, err := Resolve(w, n, models.RoleDeveloper, true, false); err != ErrOffline {
		t.Fatalf("%v", err)
	}
}
