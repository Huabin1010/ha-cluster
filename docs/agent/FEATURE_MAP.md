...
- **窄屏**：指标随列表滚动；节点为卡片（testid 仍是 `node-row` / `node-ready`）
...
按架构看空闲 CPU/内存/磁盘；刷新。只读。**窄屏**为卡片（`capacity-row`）。
...
添加私有 Registry、测连通、开关自动注入、删除。推送镜像与部署优先推荐 **CNB**（`docker.cnb.cool`）；添加仓库可一键填入推荐地址。**窄屏**为卡片（`registries-row`）。
...
添加域名后缀、启用/停用、删除；免审域可开放随机/自定义前缀。平台为每个启用后缀自动登记通配符 HTTPS 证书，可立即签发 / 自动续期。推送镜像与部署优先推荐 CNB（`docker.cnb.cool`）。**窄屏**证书与后缀均为卡片（`tls-cert-row` / `ing-zone-row`）。
...
窄屏列表走卡片、桌面走表格；同一行的 `data-testid` 不变。不要按桌面 `<th>` 去找窄屏列表。
